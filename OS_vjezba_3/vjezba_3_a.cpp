...
void* f_generiraj(void* arg) {
    ...
    for (int i = 0; i < 10; ++i) sem_post(&postavljen_broj); // 10 je dovoljno za m <= 10
...
void* f_racunaj(void* arg) {
    while (true) {
        ...
        int temp = gl_var;
        cout << "Dretva " << (int)pthread_self() << " preuzela zadatak " << temp << endl;
        ...
        cout << "Dretva " << (int)pthread_self() << " zadatak = " << temp << " zbroj = " << sum << endl;
    }
...
int main(int argc, char *argv[]) {
    ...
    pthread_create(&generiraj_id, NULL, f_generiraj, &n);
